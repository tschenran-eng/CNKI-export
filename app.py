...
try:
    ...
    raise SystemExit(
        "\n缺少 Qt 运行库。当前程序包不完整，请重新下载 Windows 便携版。\n"
    ) from exc
...
class RobustNativeExporter(CNKINativeExporter):
    ...
    async def launch(self) -> None:
        ...
        raise RuntimeError(
            "未能启动 Microsoft Edge 或 Google Chrome。"
            "请确认 Windows 已安装 Edge（通常系统自带）。\n"
            + "\n".join(errors)
        )
    ...
    async def _wait_grid_ready(self, page, expected_rows: int | None = None, timeout_ms: int = 25000) -> int:
        ...
        while asyncio.get_running_loop().time() < deadline:
            ...
            master_count = await page.locator(
                "#selectCheckAll1, input[type='checkbox'][id*='selectCheckAll']"
            ).count()
        ...
        rows = await self.current_row_count(page)
        raise RuntimeError(
            f"结果页未稳定：当前读取到 {rows} 行，或“全选/已选计数”控件尚未加载完成。"
        )
    ...
    async def _try_label(self, page, target: int) -> bool:
        labels = page.locator(
            "label.checkAll, label:has(#selectCheckAll1), label:has(input[id*='selectCheckAll'])"
        )
    ...
    async def _try_cnki_handler(self, page, target: int) -> bool:
        try:
            result = await page.evaluate(
                """() => {
                    const el = document.querySelector('#selectCheckAll1') ||
                               document.querySelector("input[type='checkbox'][id*='selectCheckAll']");
                ...
                """() => {
                    ...
                    for (const row of rows) {
                        const boxes = [...row.querySelectorAll("input[type='checkbox']")]
                            .filter(e => !String(e.id || '').includes('selectCheckAll'));
                        const cb = boxes[0];
                        if (cb && !cb.checked) {
                            cb.click();
                            clicked += 1;
    ...
    async def _dump_select_failure(self, page, before: int, rows: int, after: int) -> Path:
        ...
        try:
            ...
            pass
        try:
            checkboxes = await page.locator("input[type='checkbox']").evaluate_all(
                """els => els.map((e, i) => ({
                    index: i,
                    id: e.id || '',
                    name: e.name || '',
    ...
    async def select_current_page(self, page) -> int:
        ...
        for method in (
            self._try_master_checkbox,
            self._try_label,
            self._try_cnki_handler,
            self._try_row_checkboxes,
        ):
        ...
        raise RuntimeError(
            f"当前页全选失败：目标 {target}，实际 {after}。诊断文件：{folder}"
        )
    ...
    def _field_matches(self, meta: dict[str, Any], key: str) -> bool:
        combined = norm_field_text(
            " ".join(
                str(meta.get(x, "")) for x in ("id", "name", "value", "text")
            )
        )
    ...
    async def select_all_export_fields(self, export_page) -> None:
        ...
        if missing:
            ...
            available = [
                f"{m.get('index')}: {m.get('value','')} | {m.get('text','')[:120]}"
                for m in catalog
            ]
        ...
        selected_indices = sorted(
            {idx for indices in matched.values() for idx in indices}
        )
        ...
        checked = await inputs.evaluate_all(
            "els => els.map((e,i) => e.checked ? i : -1).filter(i => i >= 0)"
        )
        checked_set = {int(x) for x in checked}
        if not set(selected_indices).issubset(checked_set):
            raise RuntimeError(
                f"自定义导出字段勾选未完全生效：目标 {len(selected_indices)}，"
                f"实际 {len(checked_set)}。"
            )
...
class BrowserBackend:
    ...
    async def open_browser_async(self, workspace: Path) -> None:
        try:
            ...
            self.exporter = RobustNativeExporter(
                workspace, workspace / ".cnki-profile", self.ui_queue
            )
    ...
    def guard_resume_state(
        ...
        if state_path.exists():
            ...
            if existing and old_sig and old_sig != sig:
                raise RuntimeError(
                    f"集合 ID“{collection_id}”已有 {len(existing)} 个旧批次，"
                    "但当前检索式已改变。请换一个集合 ID，避免把两套结果混在一起。"
                )
            if existing and old_fields and old_fields != export_fields:
                raise RuntimeError(
                    f"集合 ID“{collection_id}”已有 {len(existing)} 个旧批次，"
                    "但当前导出字段设置与之前不同。请换一个集合 ID，"
                    "或删除旧批次后重新开始。"
                )
            ...
            json.dumps(
                {
                    ...
                    "query": query,
                    "query_signature": sig,
                    "export_fields": sorted(export_fields),
    ...
    async def run_export_async(
        ...
        *,
        ...
        try:
            ...
            manifest = await self.exporter.export_native_collection(
                collection,
...
class MainWindow(QMainWindow):
    ...
    def _build(self):
        ...
        sub = QLabel("持久登录 · 可选导出字段 · 原生 XLS · 批次断点续跑")
        ...
        dp = QLabel(
            "断点单位是已成功下载的 XLS 批次。\n\n"
            "例如 500 条一批：若在第 5 页中断，重新运行只重做当前批次；"
            "此前已经下载成功的批次会直接跳过。"
        )
    ...
    def _style(self):
        self.setStyleSheet("""
        QWidget#Root {
            background:#F4F6FA; color:#172033;
            font-family:"Segoe UI","Microsoft YaHei UI"; font-size:14px;
        }
        ...
        QFrame#FieldBox {
            background:#F8FAFC; border:1px solid #E5EAF1; border-radius:9px;
        }
        ...
        QLabel#Resume {
            background:#F7F9FC; color:#5F6C80; border:1px solid #E6EAF1;
            border-radius:8px; padding:9px 10px;
        }
        QLineEdit, QSpinBox, QPlainTextEdit#QueryEditor {
            background:#FBFCFE; color:#172033; border:1px solid #DDE3EC;
            border-radius:8px; padding:8px 10px;
        }
        QLineEdit:focus, QSpinBox:focus, QPlainTextEdit#QueryEditor:focus {
            border:1px solid #4B7CF3; background:#FFFFFF;
        }
        ...
        QPushButton {
            min-height:34px; border-radius:8px; padding:5px 14px; font-weight:600;
        }
        QPushButton[kind="primary"] {
            color:white; background:#2F6FEB; border:1px solid #2F6FEB;
        }
        QPushButton[kind="primary"]:hover { background:#255FCC; }
        QPushButton[kind="primary"]:disabled {
            background:#AEBBD2; border-color:#AEBBD2;
        }
        QPushButton[kind="secondary"] {
            color:#2B3547; background:#F7F9FC; border:1px solid #DDE3EC;
        }
        QPushButton[kind="secondary"]:hover { background:#EDF2F8; }
        QPushButton[kind="danger"] {
            color:#B42318; background:#FFF2F0; border:1px solid #FFD7D2;
        }
        QPushButton[kind="ghost"] {
            color:#5E6B80; background:transparent; border:1px solid transparent;
        }
        QProgressBar {
            background:#EAF0F7; border:none; border-radius:5px;
            min-height:10px; max-height:10px;
        }
        QProgressBar::chunk { background:#2F6FEB; border-radius:5px; }
        QPlainTextEdit#Log {
            background:#111827; color:#D9E2F2; border:1px solid #1F2937;
        ...
        QLabel[level="neutral"] {
            color:#536176; background:#EEF2F7; border:1px solid #DEE5EF;
                ...
                json.dumps(
                    {
                        ...
                        "collection_id": self.collection.text().strip(),
                        "name": self.name.text().strip(),
                        "query": self.query.toPlainText().strip(),
                        "batch_size": self.batch.value(),
                        "max_batches": self.max_batches.value(),
    ...
    def poll_events(self):
        while True:
            ...
            if kind == "log":
                ...
                self.pill.set_state("warn" if ready else "neutral",
                                    "浏览器已启动" if ready else "浏览器未启动")
            elif kind == "login":
                level, text = payload
                ...
                self.status.setText(
                    f"命中 {payload.get('result_count', 0)} 条；"
                    f"已记录 {payload.get('batches', 0)} 个批次"
                )
                self.refresh_resume()
            elif kind == "error":
                self.append_log(str(payload))
    ...
    def choose_workspace(self):
        folder = QFileDialog.getExistingDirectory(
            self, "选择工作目录", self.workspace.text().strip() or str(Path.home())
        )
        if folder:
    ...
    def open_output(self):
        try:
            path = self.workspace_path() / "output" / "native_exports" / (self.collection.text().strip() or "Q1")
    ...
    def refresh_resume(self):
        try:
            ...
            if not cid:
                ...
                return
            folder = wp / "output" / "native_exports" / cid
            ...
            if warnings:
                self.resume.setText(
                    f"断点警告：已有 {len(files)} 个 XLS 批次，但"
                    + "、".join(warnings)
                    + "。请换集合 ID，避免混用旧数据。"
                )
            else:
                self.resume.setText(
                    f"断点状态：检测到 {len(files)} 个已完成 XLS 批次，继续运行时会自动跳过。"
                )
        except Exception:
